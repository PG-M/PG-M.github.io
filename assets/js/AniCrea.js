...
document.addEventListener('DOMContentLoaded', function () {
    // 多重验证
    const hasAccess = sessionStorage.getItem('anicrea_access') === 'granted';
    const referrer = document.referrer;

    // 检查是否从主页跳转而来
    const isFromMainPage = referrer.includes('Fireworks.html') ||
    ...
    if (!hasAccess || !isFromMainPage) {
        // 如果不是正确途径访问，立即跳转回主页
        alert('请从主页正确解锁后访问！');
        sessionStorage.removeItem('anicrea_access');
...
function initAniCrea() {
  ...
  `;
    ...
    // ======== 音效选择功能 =========
    // 将音效选择功能移到主函数内，但不要嵌套DOMContentLoaded
    const soundOptions = document.querySelectorAll('.sound-option');
    let selectedSound = 'default-sound'; // 全局变量
    ...
    soundOptions.forEach(option => {
        option.addEventListener('click', function () {
            ...
            console.log('选择的音效:', selectedSound);
        });
    });
